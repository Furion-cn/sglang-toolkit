...
if [ "$PD_ROLE" = "prefill" ];then
   DISTRIBUTED_ARGS+=(
     ...
     --kv-transfer-config "{\"role\":\"prefill\",\"decode_dist_init_host\":\"$SERVER_NAME-decode-0.$SERVER_NAME-decode.inference-system.svc.cluster.local\",\"transfer_engine_metadata_server\":\"$ETCD_ADDR\",\"transfer_engine_rdma_device\":\"$KV_TRANSFER_DEVICE\"}"
     --disable-cuda-graph
   ...
   DISTRIBUTED_ARGS+=(
     ...
     --kv-transfer-config "{\"role\":\"decode\",\"prefill_dist_init_host\":\"$SERVER_NAME-prefill-0.$SERVER_NAME-prefill.inference-system.svc.cluster.local\",\"transfer_engine_metadata_server\":\"$ETCD_ADDR\",\"transfer_engine_rdma_device\":\"$KV_TRANSFER_DEVICE\"}"
     --chunked-prefill-size -1
     --disable-radix-cache
   )
   if [ "$ENABLE_DEEPEP" -eq 1 ]; then
